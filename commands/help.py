...
def setup_help(bot, prefix, command_descriptions):
    ...
    async def help_command(ctx, command_name: str = None):
        ...
        if command_name:
            ...
            # Use command.name (actual command name) to look up description, not the alias
            ...
            usage = f"`!{command.name} {signature}`" if signature else f"`!{command.name}`"
        ...
        queue_cmds = [
            ...
            f"`{prefix}nowplaying` - {command_descriptions['nowplaying']}",
        ]
        embed.add_field(
            name="📋 Queue",
            value="\n".join(queue_cmds),
        ...
        # Quick tips
        embed.set_footer(text="💡 Tip: You can use playlist URLs with !play • Supports YouTube/YouTube Music")
