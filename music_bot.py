...
import os
import re
from bot_token import BOT_TOKEN  # Import the bot token from a separate file for security
...
try:
    ...
    log("syncedlyrics not installed. Lyrics feature will be disabled. Install with: pip install syncedlyrics", "warn")
...
async def play(ctx, *, query: str):
    ...
    if webpage:
        loop = asyncio.get_event_loop()
        with yt_dlp.YoutubeDL(YDL_OPTS) as ydl:
            fresh = await loop.run_in_executor(None, lambda: ydl.extract_info(webpage, download=False))
    ...
    if vc.is_playing() or vc.is_paused():
        ...
        await ctx.send(f"➕ Added to queue (#{len(state['queue'])}): **{title}** `[{mins}:{secs:02d}]`")
    else:
        state['queue'].append(info)
...
async def lyrics(ctx, *, query: str = None):
    ...
    if not LYRICS_AVAILABLE:
        return await ctx.send("❌ Lyrics feature is not available. Install syncedlyrics: `pip install syncedlyrics`")
    ...
    if query:
        ...
        if state.get('current_song'):
            ...
            return await ctx.send("❌ No song is currently playing. Use `!lyrics <song name>` to search for a specific song.")
    ...
    try:
        ...
        lyrics_clean = re.sub(r'\[(\d{2}:\d{2}\.\d{2})\]', '', lyrics_result)  # Remove LRC timestamps
        lyrics_clean = re.sub(r'\n{3,}', '\n\n', lyrics_clean)  # Normalize excessive newlines
        ...
        await msg.edit(content=f"{header}```{chunks[0][:max_length-len(header)-len(footer)-6]}```{footer}")
...
class BotLauncher(tk.Tk):
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="🎵  Discord Music Bot",
                 font=FONT_BIG, fg="white", bg=ACCENT).pack(side="left", padx=16, pady=10)

        self.status_dot = tk.Label(header, text="●", font=("Segoe UI", 14),
                                   fg=TEXT_DIM, bg=ACCENT)
        self.status_dot.pack(side="right", padx=6)
        self.status_lbl = tk.Label(header, text="Offline",
                                   font=FONT_UI, fg=TEXT_DIM, bg=ACCENT)
        ...
        self.start_btn = tk.Button(
            bar, text="▶  Start Bot", font=FONT_BOLD,
        ...
        tk.Button(
            bar, text="🗑  Clear Log", font=FONT_UI,
            bg=BG3, fg=TEXT_DIM, activebackground=BG,
        ...
        tk.Label(bar, textvariable=self.cmd_var, font=FONT_UI,
                 fg=TEXT_DIM, bg=BG2).pack(side="right", padx=10)

        self.cmd_count = 0

        self._append_log("Welcome! Press ▶ Start Bot to connect.", "dim")
        self._append_log(f"Token: {'set ✓' if BOT_TOKEN != 'PASTE_YOUR_TOKEN_HERE' else '⚠ not set — edit bot_launcher.py'}", 
                         "success" if BOT_TOKEN != "PASTE_YOUR_TOKEN_HERE" else "error")
        
        # Check lyrics availability
        if LYRICS_AVAILABLE:
            self._append_log("Lyrics feature: Available ✓ (syncedlyrics loaded)", "success")
        else:
            self._append_log("Lyrics feature: Unavailable ✗ (pip install syncedlyrics)", "warn")
    ...
    def _set_status(self, online: bool):
        ...
        if online:
            ...
            self.start_btn.config(text="⏹  Stop Bot", bg=RED, activebackground="#c03537")
        else:
            self.status_dot.config(fg=TEXT_DIM)
            self.status_lbl.config(text="Offline", fg=TEXT_DIM)
            self.start_btn.config(text="▶  Start Bot", bg=ACCENT, activebackground="#4752c4")
    ...
    def _start_bot(self):
        ...
        if BOT_TOKEN == "PASTE_YOUR_TOKEN_HERE":
            messagebox.showerror("No Token", "Please paste your bot token into bot_launcher.py")
            return
